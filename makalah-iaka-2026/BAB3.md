...
3. **Pemeliharaan Berbasis Kondisi Riil (*Condition-Based Maintenance / CBM*):** Mengubah sistem pemeliharaan seluruh sembilan mesin cetak (Komori, Ryobi, dan GTO) dari jadwal berkala statis (*time-based*) menjadi intervensi presisi berbasis profil anomali komponen (*defect-driven maintenance*).
...
Tingkatan kedua adalah **Lapisan 2: Analisis Preskriptif & Visualisasi Aksi (*Prescriptive Analytics & Actionable Visualization Layer*)**. Lapisan ini bertindak sebagai mesin pengolah kecerdasan data (*analytics aggregation engine*). Mengingat proses sortir dan pemeriksaan mutu fisik di Unit Verifikasi (beroperasi 2 gilir) membutuhkan jeda waktu (*lead time QC*) sekitar 1–2 hari setelah proses cetak (beroperasi 3 gilir 24 jam) selesai, Lapisan 2 bertugas menjembatani jeda waktu tersebut secara otomatis. Begitu petugas verifikasi menginput data lembar Hasil Cetak Tidak Sempurna (`hcts_pikai`), sistem secara otomatis **merekonsiliasikan (*reconcile*)** data cacat tersebut dengan data penugasan di meja mesin (`transaksi_cetak`) dan target pesanan SAP ERP (`order_pcht`) berdasarkan nomor PO yang cocok.

Seluruh rekonsiliasi data historis granular tersebut kemudian disintesis menjadi empat modul visual aksi yang selalu mutakhir: **Fitur 3 (Produksi Mesin Cetak)** untuk mendeteksi anomali performa per mesin, **Fitur 4 (Produksi Unit Cetak)** untuk memetakan dinamika kinerja antar-tim dan antar-shift, **Fitur 5 (Audit Jenis Kerusakan Tiap Mesin)** untuk memandu teknisi pemeliharaan dengan diagram Pareto komponen kritis sebelum servis mesin, serta **Fitur 6 (Layar Andon Lapangan)** yang memancarkan transparansi kemajuan pesanan dan peringkat mutu ke seluruh area lini cetak.
...
1. **Sub-Fitur Template Tim Mingguan:** Berfungsi sebagai cetak biru struktur penugasan kelompok kerja tetap di setiap unit mesin cetak. Kepala Kelompok menetapkan susunan operator utama (Operator 1 dan Operator 2) untuk Tim A, Tim B, dan Tim C pada masing-masing mesin cetak, menentukan urutan gilir awal, serta mengaktifkan fungsi **Rotasi Shift Mingguan Otomatis** dengan pola standar perputaran: Pagi $\rightarrow$ Malam $\rightarrow$ Sore.
...
### 3.2.6 Fitur 6 – Andon Display Visual Area Cetak (*Shop-Floor Visual Management*)
Sebagai wujud penerapan prinsip manajemen visual di area kerja (*Visual Management / Visual Factory*), DSS SIRINE 4.0 dilengkapi dengan modul Andon Display yang ditayangkan secara terpusat pada monitor layar lebar di aula lini cetak. Modul ini beroperasi mandiri tanpa intervensi manual, dengan mekanisme pembaruan otomatis setiap **60 detik (*auto-refresh*)**.
...
1. **Layar 1 (Ringkasan Produksi & Status Pesanan):** Memancarkan metrik utama produksi unit, mencakup akumulasi lembar cetak berjalan (4.102.040 LK), hasil cetak sempurna / HCS (2.378.421 LK), lembar rusak / HCTS terverifikasi (105.169 LK), dan tingkat *inschiet* unit (2,56%). Layar ini juga memuat daftar pesanan yang mendekati tenggat waktu serta diagram lingkaran 4 jenis kerusakan dominan dari data pesanan yang telah selesai diverifikasi (Noda 48,5%, Zig-zag 24,8%, Blobor 16,4%, dan Tipis/Tebal 6,0%).
2. **Layar 2 (Komparasi Performa Seluruh Mesin):** Menyajikan perbandingan visual *head-to-head* volume lembar cetak dan tingkat *inschiet* antar 9 mesin cetak (Komori 1–4, Ryobi 1–2, dan GTO 1–3) berdasarkan data pesanan terverifikasi. Layar ini langsung menunjukkan mesin dengan mutu terbaik (Komori 2 dengan *inschiet* 0,12%–3,43%) serta mesin yang memerlukan pengawasan ketat (Ryobi 2 dengan *inschiet* 4,50%–5,87%).
3. **Layar 3 (Drill-Down Mesin Kritis):** Menyoroti secara khusus profil mesin yang sedang mengalami deviasi performa (seperti Ryobi 2 dengan total cetak 595.082 LK dan kerusakan 26.798 LK), lengkap dengan diagram donat proporsi cacat dominannya (Noda 44% dan Blobor 21,5%) dari hasil verifikasi agar menjadi perhatian pengawas dan teknisi saat evaluasi harian.
...
*Gambar 3.13: Layar Andon 1 – Ringkasan Produksi Mingguan, Status Inschiet Terverifikasi 2,56%, Peringatan OBC Mendekati Jatuh Tempo, dan Kerusakan Dominan (Sumber: Layar Monitor Andon Unit Cetak)*

![Layar Andon 2: Komparasi Produksi dan Inschiet Antar-Mesin](../extracted_images/image19.png)
*Gambar 3.14: Layar Andon 2 – Matriks Komparasi Volume Lembar Cetak, HCS, HCTS, dan Persentase Inschiet Terverifikasi Antar-Mesin Cetak (Sumber: Layar Monitor Andon Unit Cetak)*

![Layar Andon 3: Detail Performa dan Kerusakan Mesin Individu Ryobi 2](../extracted_images/image20.png)
*Gambar 3.15: Layar Andon 3 – Visualisasi Status Khusus Mesin Ryobi 2 dengan Diagram Donat Kerusakan Dominan Terverifikasi (Sumber: Layar Monitor Andon Unit Cetak)*
...
> Layar Andon menghadirkan transparansi total di lini produksi cetak. Kepala *Shift* dan Operator tidak perlu membuka komputer untuk mengetahui kondisi kemajuan produksi. Pada Gambar 3.16, sistem menampilkan **kode OBC kritis (seperti MLG544772 yang terlambat -1 hari dan MAD544789 yang sisa 1 hari)** dengan kode warna mencolok, memungkinkan tim gilir memprioritaskan pencetakan pesanan mendesak tanpa melanggar Service Level Agreement (SLA) DJBC Kemenkeu RI.
...
Kelelahan fisik akibat ritme sirkadian (*circadian fatigue*) pada *Shift* Malam (pukul 23.00–07.00 WIB) serta disparitas keterampilan teknis antar-operator sebelumnya menjadi kendala yang sulit diintervensi karena adanya jeda waktu pemeriksaan 1–2 hari di Unit Verifikasi tanpa didukung pencatatan penugasan yang tertib di meja mesin. Kehadiran **Fitur 1 (Form Konfirmasi PO Digital)** dan **Fitur 4 (Produksi Unit Cetak)** memberikan solusi terpadu. Formulir digital memudahkan operator mencatat transaksi penugasan dalam hitungan detik. Ketika hasil pemeriksaan mutu dari Unit Verifikasi selesai diinput 1–2 hari kemudian, sistem secara otomatis menautkan data kerusakan ke tim dan *shift* yang mencetaknya. Data kinerja objektif ini menjadi bahan evaluasi pada *Daily Production Meeting*, memungkinkan Kepala Unit memberikan pendampingan teknis (*coaching*) yang terarah kepada operator yang mengalami deviasi mutu tanpa bias subjektif. Sementara itu, Layar Andon (Fitur 6) di aula cetak menjaga kesadaran bersama terhadap target pesanan dan peringatan batas waktu jatuh tempo pesanan (*SLA delivery*).
